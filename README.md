# Miniguia de Estudo em Finanças Introdutórias com NotebookLM

## 📌 Introdução
Este projeto foi desenvolvido como parte de um desafio de aprendizagem ativa do **bootcamp Bradesco – GenAI & Dados (DIO)**. Ele reúne fontes abertas sobre finanças introdutórias no NotebookLM, com o objetivo de criar um miniguia de estudo que apoie revisões futuras e incentive o uso da IA como ferramenta de aprendizagem crítica e organizada.

## 🎯 Objetivos de Estudo
- Compreender conceitos básicos de finanças pessoais.  
- Identificar termos-chave e criar um glossário.  
- Elaborar perguntas estratégicas para fixação.  
- Testar variações de prompts e registrar respostas com referências.  

---

## ❓ Perguntas Estratégicas
1. Qual a diferença entre orçamento pessoal e orçamento familiar?  
2. Como o planejamento financeiro contribui para evitar endividamento?  
3. O que significa liquidez e por que é importante em finanças pessoais?  
4. Quais são os principais erros comuns na gestão de finanças pessoais?  
5. Como aplicar os conceitos de juros compostos em investimentos de longo prazo?  

---

## 💡 Variações de Prompts para NotebookLM

- **Explicação simples**  
  - "Explique em linguagem acessível o que é orçamento familiar e dê um exemplo prático."  

- **Comparação**  
  - "Compare orçamento pessoal e orçamento familiar, destacando semelhanças e diferenças."  

- **Aplicação prática**  
  - "Mostre como aplicar o conceito de planejamento financeiro para organizar despesas mensais."  

- **Crítica e reflexão**  
  - "Quais riscos existem quando uma pessoa não faz planejamento financeiro? Use exemplos reais."  

- **Resumo rápido**  
  - "Liste em tópicos os principais conceitos de finanças pessoais apresentados pelo Banco Central e pela CVM."  

- **Glossário expandido**  
  - "Crie um glossário com 10 termos fundamentais de finanças pessoais, com definições curtas."  

---

## 📚 Fontes Utilizadas
- [Banco Central do Brasil – Caderno de Educação Financeira: Gestão de Finanças Pessoais (PDF)](https://www.bcb.gov.br/content/cidadaniafinanceira/documentos_cidadania/Cuidando_do_seu_dinheiro_Gestao_de_Financas_Pessoais/caderno_cidadania_financeira.pdf)  
- [GUIA CVM – Planejamento Financeiro (PDF)](https://www.gov.br/investidor/pt-br/educacional/publicacoes-educacionais/guias/guia-de-planejamento-financeiro/guia-planejamento-financeiro.pdf)  
- [FIA – Educação Financeira (Página Web)](https://fia.com.br/blog/educacao-financeira/)  

---

## 🧩 Estrutura do Miniguia
- **Resumos estruturados**: síntese dos principais tópicos estudados.  
- **Glossário**: termos como “juros compostos”, “orçamento”, “investimento”.  
- **Prompts reutilizáveis**: conjunto de perguntas estratégicas para apoiar revisões futuras.  

## ⚙️ Uso da IA
O NotebookLM foi utilizado para:  
- Curadoria das fontes.  
- Criação de perguntas estratégicas.  
- Teste de variações de prompts.  
- Registro das respostas e referências.  

## 🚀 Como reutilizar
Outros estudantes podem usar este miniguia para revisar conceitos básicos de finanças e aplicar os prompts em novos contextos de estudo.  

## 📄 Licença
Este projeto está sob a licença MIT. Sinta-se livre para reutilizar e adaptar o conteúdo para fins educacionais.  

---
Este repositório foi desenvolvido como parte da atividade prática do **bootcamp Bradesco – GenAI & Dados (DIO)**. Além de cumprir os objetivos da entrega, ele pode servir como referência e apoio para outros participantes que desejam revisar conceitos de finanças introdutórias e explorar o uso da IA como ferramenta de aprendizagem ativa.
